"""Learning-rate schedules with a linear warmup.

``CosineAnnealingLRWithWarmup`` is the one used in the paper: the LR ramps from
``base_lr`` to ``warmup_lr`` over ``warmup`` optimiser steps, then cosine-decays
to ``min_lr`` over ``T_max`` steps.
"""
...
__all__ = ["CosineAnnealingLRWithWarmup", "ReduceLROnPlateauWithWarmup"]
